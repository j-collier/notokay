...
type streamConn struct {
	...
	// this will be exponentially increased until
	// reaching maxWait, when it will exits
	wait    int
	maxWait int
...
func (conn *streamConn) Close() {
	// marks connection as stale and lets connect()
	// handler close after a read
	conn.mu.Lock()
...
func readStream(client *twittergo.Client, sc streamConn, path string, query url.Values,
	...
	for {
		...
		if err != nil {
			...
			// try reconnection, exponentially back off until MaxWait
			// is reached then exit?
			resp, err := Connect(client, path, query)
			if err != nil || resp == nil {
				fmt.Println("Could not reconnect to source. Sleeping then retry... ")
				if sc.wait < sc.maxWait {
					sc.wait = sc.wait * 2
				} else {
					fmt.Println("Max wait reached, exiting... ")
					done <- true
					return
				}
				continue
